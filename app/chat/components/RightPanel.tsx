...
const RightPanel = ({ onSelectChat, onNewChat }: RightPanelProps) => {
	const [chatData, setChatData] = useState<Chat[]>(initialData.chatData);
	...
	const handleNewChat = () => {
		...
		// Add the new chat to the list and sort by latest
		setChatData((prevChatData) => {
			const updatedChats = [...prevChatData, newChat].sort(
				(a, b) => new Date(b.lastSeen).getTime() - new Date(a.lastSeen).getTime()
			);
			return updatedChats;
		});

		// Call the `onNewChat` function to open this new chat
		onNewChat(newChat);
	};
                                ...
                                <PopoverButton className="flex items-start space-x-4 py-2">
                                    ...
                                    <section className="space-y-2 text-left">
                                        ...
                                        <section className="flex justify-between items-center">
                                            ...
                                            <p className="text-xs font-normal text-gray-400">{chat.lastSeen}</p>
                                        </section>
                                    </section>
                                </PopoverButton>
							</section>
